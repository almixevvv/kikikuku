<?php if(!defined("BASEPATH")) exit("Hack Attempt");
  ...
  class PaymentProcess extends CI_Controller {
    ...
    public function manualProcess() {
      ...
      $path = $_FILES['payment-receipt']['name'];
      ...
      if ( !$this->upload->do_upload('payment-receipt')) {
        ...
        $data = array(
          'ORDER_ID' => $orderID,
          'ACCOUNT_NAME' => $this->input->post('payment-name'),
          'ACCOUNT_NUMBER' => $this->input->post('account-number'),
          'ACCOUNT_BANK' => $this->input->post('payment-bank'),
          'PAYMENT_AMOUNT' => $this->input->post('payment-number'),
          'PAYMENT_ID' => $this->input->post('payment-number'),
